...
sqlite3_vfs *sqlite3_vfs_find(const char *zVfs){
  ...
  LOG_PRINTF(("sqlite3_vfs_find 1"));
  sqlite3_mutex_enter(mutex);
  LOG_PRINTF(("sqlite3_vfs_find 2"));
  if( !isInit ){
	  LOG_PRINTF(("sqlite3_vfs_find 3"));
    vfsList = sqlite3OsDefaultVfs();
    isInit = 1;
  }

  LOG_PRINTF(("sqlite3_vfs_find 4 vfsList %d", vfsList));
  for(pVfs = vfsList; pVfs; pVfs=pVfs->pNext){
	  LOG_PRINTF(("sqlite3_vfs_find 5 pVfs %d", pVfs));

    if( zVfs==0 ) break;
    if( strcmp(zVfs, pVfs->zName)==0 ) break;
  }
  LOG_PRINTF(("sqlite3_vfs_find 6"));
  sqlite3_mutex_leave(mutex);
  LOG_PRINTF(("sqlite3_vfs_find 7"));
  return pVfs;
}
