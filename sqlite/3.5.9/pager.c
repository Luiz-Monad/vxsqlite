...
int sqlite3PagerPagecount(Pager *pPager){
  ...
  LOG_PRINTF(("sqlite3PagerPagecount 1"));
  assert( pPager!=0 );
  if( pPager->errCode ){
    return -1;
  }

  LOG_PRINTF(("sqlite3PagerPagecount 2"));

  if( pPager->dbSize>=0 ){
    n = pPager->dbSize;
	LOG_PRINTF(("sqlite3PagerPagecount 3"));
  } else {
    assert(pPager->fd->pMethods||pPager->tempFile);
    if( (pPager->fd->pMethods)
     && (rc = sqlite3OsFileSize(pPager->fd, &n))!=SQLITE_OK ){

		 LOG_PRINTF(("sqlite3PagerPagecount 4"));
      pPager->nRef++;
      pager_error(pPager, rc);
      pPager->nRef--;
      return -1;
	...
	LOG_PRINTF(("sqlite3PagerPagecount 5"));
	...
	LOG_PRINTF(("sqlite3PagerPagecount 6"));
    if( pPager->state!=PAGER_UNLOCK ){
      pPager->dbSize = n;
    }
  }

  LOG_PRINTF(("sqlite3PagerPagecount 7"));
  if( n==(PENDING_BYTE/pPager->pageSize) ){
    n++;
  }

  LOG_PRINTF(("sqlite3PagerPagecount 8"));
  if( n>pPager->mxPgno ){
    pPager->mxPgno = n;
  }

  LOG_PRINTF(("sqlite3PagerPagecount return %d", n));
  return n;
}
