...
static int lockBtree(BtShared *pBt){
  ...
  */
  LOG_PRINTF(("lockBtree 2"));
  rc = SQLITE_NOTADB;
  nPage = sqlite3PagerPagecount(pBt->pPager);
  LOG_PRINTF(("lockBtree 3"));
  if( nPage<0 ){
	  LOG_PRINTF(("lockBtree 4 nPage<0"));
    rc = SQLITE_IOERR;
	...
	LOG_PRINTF(("lockBtree 5"));

    if( page1[18]>1 ){
      pBt->readOnly = 1;
    }
	
	LOG_PRINTF(("lockBtree 6"));

    if( page1[19]>1 ){
      goto page1_init_failed;
    }

	LOG_PRINTF(("lockBtree 7"));
...
page1_init_failed:
  ...
  LOG_PRINTF(("lockBtree 11 rc %d", rc));
  releasePage(pPage1);
  pBt->pPage1 = 0;
  return rc;
}
